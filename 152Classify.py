...
#        origImages = np.load("Test_Images.npy")
#        return origImages
    origImages = []
    for i in os.listdir(sourceDir):
        ...
        for filename in tqdm(os.listdir(folderName)):
            ...
            origImages.append([image, image_number])
    print("\nNumber of images read:", len(origImages))
    return origImages

def predict_images(testSet):
    predictions = []
    for i in tqdm(testSet):
        image = []
        image.append(i[0])
        label = np.argmax(model.predict(np.array(image)))
        predictions.append([i[1],label])
    return predictions

def write_CSV(array):
    file = open('RN152B.csv', 'w')
...
predictions = predict_images(testSet)
